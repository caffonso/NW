...
class neurowood():                                                  # Main class
    ...
    def ClassifyFast(model,args):                                # Run classification 
      ...
      while(run):
        ...
        #trigger = 1
        if trigger:
          Class  = neurowood.CNN(im,model,args,minX,minY,run1)          # call CNN
          print('Class:',Class) 
          if args.Siemens == "on":  
    ...
    def showEnhance(im0,args,minX,minY):                        # show image bondary
      ...
      d1.text((10,130), "threshold : "+str(args.th ), font = font)  

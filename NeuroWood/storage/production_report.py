...
class ProductionReport:
    ...
    def add_board(self, sequence, pipeline, result, camera=None):
        ...
        limits = acceptance.get("limits", {}) or {}
        ...
        row = {
            ...
            "small_defects": int(counts.get("small", 0) or 0),
            "medium_defects": int(counts.get("medium", 0) or 0),
            "large_defects": int(counts.get("large", 0) or 0),
            "max_small": int(limits.get("small", 0) or 0),
            "max_medium": int(limits.get("medium", 0) or 0),
            "max_large": int(limits.get("large", 0) or 0),
            "frames_analyzed": int(summary.get("frames_analyzed", 0) or 0),
            "frames_with_defect": int(summary.get("frames_with_defect", 0) or 0),
            "defect_area_pct": _round_or_none(summary.get("defect_area_pct", 0.0), 4),
    ...
    def summary(self):
        ...
        return {
            ...
            "reject_rate_pct": 100.0 * len(rejected) / count if count else 0.0,

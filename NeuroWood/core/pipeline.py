...
from vision.width_measurement import measure_board_width
...
class NeuroWoodPipeline:
    ...
    A largura é medida apenas no primeiro frame original, antes de crop/resize.
    """
    ...
    def run_once(self):
        ...
        # --------------------------------------------------------------
        # LARGURA: sempre no frame original.
        # Na produção simulada, usa a escala definida pela própria câmera.
        # Em câmera física, usa vision.width_pixels_per_mm.
        # --------------------------------------------------------------
        pixels_per_mm = float(
            getattr(
                self.camera,
                "pixels_per_mm",
                _cfg(self.config, "vision.width_pixels_per_mm", 2.0),
            )
        )

        background_intensity = getattr(
            self.camera,
            "background_intensity",
            None,
        )

        width_background_path = _cfg(
            self.config,
            "vision.width_background_path",
            None,
        )

        try:
            width_info = measure_board_width(
                first_frame,
                pixels_per_mm=pixels_per_mm,
                background=width_background_path or None,
                background_intensity=background_intensity,
                diff_threshold=float(
                    _cfg(self.config, "vision.width_diff_threshold", 25.0)
                ),
                sample_count=int(
                    _cfg(self.config, "vision.width_sample_count", 50)
                ),
            )
            piece_info.update(width_info)
        except Exception as exc:
            logger.warning("WIDTH_MEASUREMENT_FAILED: %s", exc)
            piece_info.update(
                {
                    "width_mm": None,
                    "width_px": None,
        ...
        logger.info(
            "PIECE_DETECTED width_mm=%s width_std_mm=%s valid_samples=%s",
            piece_info.get("width_mm"),
            piece_info.get("width_std_mm"),
            piece_info.get("width_valid_samples"),
        )
        ...
        for patch in summary["patches"]:
            ...
            for defect in patch["defects"]:
                ...
                try:
                    params = inspect.signature(Defect).parameters
                except (TypeError, ValueError):
                    params = {}

                if "bbox" in params:
                    kwargs["bbox"] = defect.get("bbox")
